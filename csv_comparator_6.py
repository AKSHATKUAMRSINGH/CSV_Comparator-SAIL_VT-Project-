...
class CSVComparatorApp:
    ...
    def create_table_tabs(self):
        ...
        for key in ['diff', 'new']:
            ...
            # Add vertical lines by using style map (only appearance in ttk is limited)
            # So we simulate vertical grid lines by setting padding and border on columns
            style = ttk.Style()
    ...
    def update_tabs(self):
        for tab_key, tree in self.tables.items():
            ...
            for row in self.data[tab_key]:
                ...
                # Insert a separator line after a change in Employee Key (except after first)
                if last_key is not None and row['key'] != last_key:
                    # Insert a separator row (with blank fields but with a black border line)
                    # Since Treeview does not support direct horizontal lines, use tags and style
                    # Insert a "separator" row with a custom tag for style
                    sep_id = tree.insert('', 'end', values=('', '', '', ''))
                    tree.item(sep_id, tags=('separator',))
                last_key = row['key']

            # Configure tag to simulate a bold black horizontal line separator by changing background
            tree.tag_configure('separator', background='black')
            
            # Add vertical lines via style is limited in ttk, but we can use the 'show' option 'headings'
            # and column configuration to simulate vertical grid lines by "border" on columns, but tkinter ttk does not
            # have direct support to add vertical grid lines in Treeview, so another workaround is to set the 
            # Treeview borders and color to simulate.
    ...
    def update_table_filter(self, tab_key):
        ...
        for row in self.data[tab_key]:
            if any(query in str(v).lower() for v in row.values()):
                values = (row['key'], row['field'], row['old'], row['new'])
                tree.insert('', 'end', values=values)
                # Insert separator if employee key changes and not first matched item
                if last_key is not None and row['key'] != last_key:
                    sep_id = tree.insert('', 'end', values=('', '', '', ''))
                    tree.item(sep_id, tags=('separator',))
                last_key = row['key']
    ...
    def export_to_pdf(self, tab_key):
        ...
        try:
            ...
            for row in self.data[tab_key]:
                ...
                # If employee key changes, add a blank row to create a separation line
            ...
            table_style = TableStyle([
                ...
                ('GRID', (0, 0), (-1, -1), 0.25, colors.black),  # Adds vertical and horizontal grid lines
            ])

            # Add a bold black horizontal line to separate different employee keys
            for i in range(1, len(data)):
                if i < len(data) - 1:
                    # Check if next row is blank (separator row)
                    if data[i+1] == ['', '', '', '']:
    ...
    def send_email(self, tab_key):
        ...
        pdf_path = filedialog.askopenfilename(title=f"Select {tab_key.title()} PDF to send", filetypes=[("PDF files", "*.pdf")])
        if not pdf_path:
            return
